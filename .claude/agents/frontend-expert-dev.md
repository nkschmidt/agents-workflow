...
> Конкретный стек фронтенда проекта (фреймворк роутинга, рантайм/деплой, auth, UI-кит, библиотеки форм и таблиц, их версии и ограничения) — в твоей памяти `<корень проекта>/memory/frontend-expert-dev/`, читается на старте. Не предполагай стек по умолчанию — сверяйся с памятью и `PROJECT.md`.
