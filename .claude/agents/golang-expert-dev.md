...
> Конкретный стек бэкенда проекта (фреймворки, БД, брокеры, внешние сервисы, структура репозитория) — в твоей памяти `memory/golang-expert-dev/`, читается на старте. Сверяйся с памятью и `PROJECT.md`, не предполагай по умолчанию.
...
Память ведёшь в `memory/golang-expert-dev/` по правилам `memory/README.md` (см. также §7 CLAUDE.md). При старте сессии читаешь `memory/_shared/MEMORY.md` + `memory/golang-expert-dev/MEMORY.md`. При изменениях Go-кода обновляешь память — это часть задачи (§6 регламента).
